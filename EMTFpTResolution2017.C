...
#include "TGraph.h"
//=**************************************************************************************
//=This version includes Darin's suggestions on the study after report on 08/29/2016 EMTF meeting
//=Also, it add the eta requirement start from 1.2 and plot (1/ EMTF pT-RECO 1/pT)/ (1/RECO pT)
//=which is RECO pT/EMTF pT -1, this should not have long tail for the relative residual plot
//=**************************************************************************************
void EMTFpTResolution_v10()
{
    ...
    /*
    TH1F *unexpected_peak_EMTF_modes = new TH1F("Unexpected peak EMTF modes","Unexpected peak EMTF modes",16,0,16);
    TH1F *unexpected_peak_EMTF_pT = new TH1F("Unexpected peak EMTF pT","Unexpected peak EMTF pT",300,0,300);
    ...
    for(Long64_t iEntry = 0; iEntry <numEvents; iEntry++){
        ...
        if(TrkPt->size()!=0 || RecoPt->size()!=0){
            ...
            while (matched_tracks != TrkPt->size() && matched_tracks != RecoPt->size()) {
                ...
                //=================
                //version 10: add interested eta area cut here >1.2
                //================================================
                if(min_dR < 0.25 && trkBx[min_dR_itrk]==0 && fabs(recoEta[min_dR_jreco])>=1.2 && fabs(recoEta[min_dR_jreco])<=2.4){
                    ...
                    //==========================
                    //Main change from version 9
                    //==========================
                    //this is the residual curvature, not residual pT, is EMTF trk curvature - RECO trk curvature
                    Double_t Residual = -1/recoPt[min_dR_jreco]+1/trkPt[min_dR_itrk];//this changes from v9
                    Double_t Relative_Residual = Residual/(1/recoPt[min_dR_jreco]);//this is actually reco/emtf - 1, as suggested by Darin, change from v9;
                    ...
                    if(Relative_Residual< -0.7 && Relative_Residual> -1){
                        ...
                        //Fill EMTF mode
                        unexpected_peak_EMTF_modes->Fill(trkMode[min_dR_itrk]);
                        
                        //Fill eta
                        unexpected_peak_RECO_eta->Fill(fabs(recoEta[min_dR_jreco]));
                        
                        //Fill pT
                        unexpected_peak_EMTF_pT->Fill(trkPt[min_dR_itrk]);
...
TFile myPlot("/afs/cern.ch/work/w/wshi/public/EMTFpTResolution/EMTFpTResolution_274440_443_v10_lxplus.root","RECREATE");
//TFile myPlot("~/Documents/EMTFpTResolution/EMTFpTResolution_274440_443_v10_local.root","RECREATE");
...
unexpected_peak_EMTF_pT->Write();
unexpected_peak_EMTF_modes->Write();
*/
    //=******************************
    ...
    myPlot.Close();
    cout<<"---------------"<<endl;
    cout<<"All plots saved in EMTFpTResolution_274440_443_v10_lxplus.root file!"<<endl;
