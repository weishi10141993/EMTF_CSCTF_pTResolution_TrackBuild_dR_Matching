...
void CSCTFpTResolution_v10()
{
    ...
    myTree->SetBranchAddress("leg_trkBx",&TrkBx);//require csctf tracks in BX=0
    ...
    /*
    TH1F *unexpected_peak_CSCTF_modes = new TH1F("Unexpected peak CSCTF modes","Unexpected peak CSCTF modes",16,0,16);
    TH1F *unexpected_peak_CSCTF_pT = new TH1F("Unexpected peak CSCTF pT","Unexpected peak CSCTF pT",300,0,300);
    TH1F *unexpected_peak_RECO_pT = new TH1F("Unexpected peak RECO pT","Unexpected peak RECO pT",500,0,500);
    TH1F *unexpected_peak_RECO_eta = new TH1F("Unexpected peak RECO Eta","Unexpected peak RECO Eta",30,0,3);
    */
    ...
    TH1F *Relative_Residual_185_eta_24_plot_RECO_10_30 = new TH1F("Relative Residual 1.88<eta<2.4 10<RECO<30","Relative Residual 1.88<eta<2.4 10<RECO<30", 700, -2, 5);
    ...
    //=*********************
    TH1F *Unmatched_csctf_pT_plot = new TH1F("Unmatched csctf pT","Unmatched csctf pT", 100, 0, 305);
    ...
    Long64_t all_unwell_matched_csctf_tracks=0;
    ...
    for(Long64_t iEntry = 0; iEntry <numEvents; iEntry++){
        ...
        vector<int> trkBx= *TrkBx;//csctf bx
        ...
        */
        ...
        //match csctf with RECO in one event
        ...
        if(TrkPt->size()!=0 || RecoPt->size()!=0){
            ...
            int unwell_matched_csctf_tracks=0;//unwell matched csctf trks in one event
            ...
            while (matched_tracks != TrkPt->size() && matched_tracks != RecoPt->size()) {
                ...
                //apply dR cut for well matched trks, also require csctf track at BX=0
                if(min_dR < 0.25 && trkBx[min_dR_itrk]==0 && fabs(recoEta[min_dR_jreco])>=1.2 && fabs(recoEta[min_dR_jreco])<=2.4){
                    //count well matched trks
                    well_matched_tracks++;
                    trk_flag[min_dR_itrk]=2;
                    reco_flag[min_dR_jreco]=2;
                    //cout<<"Well matched trks: dR = "<< min_dR <<", trk # = "<< min_dR_itrk <<", reco # = "<< min_dR_jreco <<endl;
                    
                    //this is the residual curvature, not residual pT, is is csctf trk curvature - RECO trk curvature
                    Double_t Residual = -1/recoPt[min_dR_jreco]+1/trkPt[min_dR_itrk];
                    Double_t Relative_Residual = Residual/(1/recoPt[min_dR_jreco]);//this is actually reco/csctf - 1, as suggested by andrew;
                    ...
                    /*
                    //=====================================================================================
                    //add by Wei Shi for version 9 @08262016 to look into the unexpected peak at -1 ~ -0.7
                    //mode, eta and pT distribution
                    //=====================================================================================
                    if(Relative_Residual< -0.7 && Relative_Residual> -1){
                        
                        //Fill CSCTF mode
                        unexpected_peak_CSCTF_modes->Fill(trkMode[min_dR_itrk]);
                        
                        //Fill eta
                        unexpected_peak_RECO_eta->Fill(fabs(recoEta[min_dR_jreco]));
                        
                        //Fill pT
                        unexpected_peak_CSCTF_pT->Fill(trkPt[min_dR_itrk]);
                        
                        //
                        unexpected_peak_RECO_pT->Fill(recoPt[min_dR_jreco]);
                        
                    }//end if look at unexpected peak in Relative residual plot on (-1,-0.7)
                     */
                    //=******************************************
            ...
            for(int i=0;i<RecoPt->size();i++){
                ...
                if(reco_flag[i]!=2){//unmatched reco mark
            ...
            for(int i=0;i<TrkPt->size();i++){
                ...
                if(trk_flag[i]!=2){//unmatched csctf mark
                    
                    unwell_matched_csctf_tracks++;
            ...
            all_unwell_matched_csctf_tracks+=unwell_matched_csctf_tracks;
            
            //cout<<unwell_matched_reco_tracks<<" unwell matched RECO track(s)!"<<endl;
            //cout<<unwell_matched_csctf_tracks<<" unwell matched csctf track(s)!"<<endl;
    ...
    cout<<all_unwell_matched_csctf_tracks<<" total unwell-matched csctf tracks!"<<endl;
    ...
    missing_csctf_tracks=all_csctf_tracks - all_well_matched_tracks - all_unwell_matched_csctf_tracks;
    ...
    TFile myPlot("/afs/cern.ch/work/w/wshi/public/EMTFpTResolution/CSCTFpTResolution_274440_443_v10_lxplus.root","RECREATE");
    //TFile myPlot("~/Documents/EMTFpTResolution/CSCTFpTResolution_274440_443_v10_local.root","RECREATE");
    
    Relative_Residual_all_modes->Write();
    Delta_R_all_modes->Write();
   /*
    unexpected_peak_RECO_pT->Write();
    unexpected_peak_RECO_eta->Write();
    unexpected_peak_CSCTF_pT->Write();
    unexpected_peak_CSCTF_modes->Write();
    */
    //=******************************
    ...
    //=*******************************
    //=*Unmatched csctf and RECO plots*
    //=*******************************
    ...
    myPlot.Close();
    cout<<"---------------"<<endl;
    cout<<"All plots saved in CSCTFpTResolution_274440_443_v10_lxplus.root file!"<<endl;
