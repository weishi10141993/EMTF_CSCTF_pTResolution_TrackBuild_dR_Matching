...
//=****************************************************************
//=Wei Shi added @ 08152016
//=v3 macro further requires segment to LCT matching besides dR cut
//=and BX =0 requirements on tracks in v2 before comparing notes for
//=CSCTF and EMTF.
//===============
//=v3 also looks into the pT info and eta distribution for tracks in
//=disagreed modes of EMTF and CSCTF.
//=================================================================
//=Besides, v2 has considered duplicate LCTs when looking into
//=disagreed modes for EMTF and CSCTF.
//=****************************************************************
...
void TrackBuild_v4()
{
    ...
    cout<<"run on lxplus: "<<run_lxplus<<", run Events: "<<run_Events<<endl;
    
    if(run_lxplus==1){
        input_fileName= "/afs/cern.ch/work/w/wshi/public/EMTFpTResolution/EMTF_NTuple_274440_443_v3.root";
        //output file
        output_fileName= "/afs/cern.ch/work/w/wshi/public/EMTFpTResolution/TrackBuild_v3_274440_443_lxplus.root";
    }
    else if(run_lxplus==0){
        input_fileName = "~/Documents/EMTFpTResolution/EMTF_NTuple_274440_443_v3.root";
        //output file
        output_fileName = "~/Documents/EMTFpTResolution/TrackBuild_v3_274440_443_local.root";
    }
    else{
        cout<<"Error: Fail to locate running machine!"<<endl;
    }
    
    cout<<"input filepath: "<<input_fileName<<endl;
    cout<<"output filepath: "<<output_fileName<<endl;
    
    TFile* myFile = new TFile(input_fileName);
    cout<<"read input file..."<<endl;
    TTree* myTree = (TTree*) myFile->Get("ntuple/tree");
    myTree->Print();
    cout<<"read tree..."<<endl;
    TFile myPlot(output_fileName,"RECREATE");
    ...
    */
    ...
    //======================================================
    //{{{
    //trkLct_station, leg_trkLct_station are all [8][4]/I arrays with integer # of staions, they store at most 8 Lcts's information for one event
    ...
    //=============================
    //=add for version 3.0 08152016
    //=define pointers to pull out phi and eta for LCTs and segments
    //=the dR cut just considers the Eta and Phi for RECO muon at IP
    //=need to consider the eta and phi at endcap, which is why we
    //=pull out Eta and Phi of Lcts/segments here
    //====================================================================================
    //Found Lct_eta and Phi of CSCTF are registered zero in NTuple. And the dR cut is enough
    //to put Lct eta and phi to 0.1 precision
    //no need to propagate the muons to endcap
    //=========================================
    
    //{{{
    /*
    //Not Necessary
    float trkLct_globPhi[8][4];
    float trkLct_eta[8][4];
    float leg_trkLct_globPhi[8][4];
    float leg_trkLct_eta[8][4];
    float recoCscSeg_glob_phi[12][16];
    float recoCscSeg_glob_eta[12][16];
     */
    //}}}
    ...
    //===============================
    //=added for version 3.0 08152016
    //===============================
    //{{{
    /*
    //Not necessary
    myTree->SetBranchAddress("trkLct_globPhi",&trkLct_globPhi);
    myTree->SetBranchAddress("trkLct_eta",&trkLct_eta);
    myTree->SetBranchAddress("leg_trkLct_globPhi",&leg_trkLct_globPhi);
    myTree->SetBranchAddress("leg_trkLct_eta",&leg_trkLct_eta);
    myTree->SetBranchAddress("recoCscSeg_glob_phi",&recoCscSeg_glob_phi);
    myTree->SetBranchAddress("recoCscSeg_glob_eta",&recoCscSeg_glob_eta);
     */
    //}}}
    ...
    //==========================
    //=add by Wei Shi @ 08162016
    //=Also in fututre will present pT assignment study under Track build requirement:
    //=EMTF and RECO matched to same RECO muon, compare with previuos Relative residual study;
    //=also look into Residual for agree mode and disagree mode
    //=======================================================================================
    //--------------------------------------------------------------------------------
    //Relative Residual plots are for the sake of comparision with pT assignment study
    //--------------------------------------------------------------------------------
    //{{{
    TH1F *EMTF_Relative_Residual_all_modes = new TH1F("EMTF Relative Residual all modes","EMTF Relative Residual all modes", 700, -2, 5);
    //TH1F *EMTF_Relative_Residual_mode_15 = new TH1F("EMTF Relative Residual mode 15","EMTF Relative Residual mode 15", 700, -2, 5);
    //TH1F *EMTF_Relative_Residual_mode_14 = new TH1F("EMTF Relative Residual mode 14","EMTF Relative Residual mode 14", 700, -2, 5);
    //TH1F *EMTF_Relative_Residual_mode_13 = new TH1F("EMTF Relative Residual mode 13","EMTF Relative Residual mode 13", 700, -2, 5);
    //TH1F *EMTF_Relative_Residual_mode_12 = new TH1F("EMTF Relative Residual mode 12","EMTF Relative Residual mode 12", 700, -2, 5);
    //TH1F *EMTF_Relative_Residual_mode_11 = new TH1F("EMTF Relative Residual mode 11","EMTF Relative Residual mode 11", 700, -2, 5);
    //TH1F *EMTF_Relative_Residual_mode_10 = new TH1F("EMTF Relative Residual mode 10","EMTF Relative Residual mode 10", 700, -2, 5);
    //TH1F *EMTF_Relative_Residual_mode_9 = new TH1F("EMTF Relative Residual mode 9","EMTF Relative Residual mode 9", 700, -2, 5);
    //TH1F *EMTF_Relative_Residual_mode_7 = new TH1F("EMTF Relative Residual mode 7","EMTF Relative Residual mode 7", 700, -2, 5);
    //TH1F *EMTF_Relative_Residual_mode_6 = new TH1F("EMTF Relative Residual mode 6","EMTF Relative Residual mode 6", 700, -2, 5);
    //TH1F *EMTF_Relative_Residual_mode_5 = new TH1F("EMTF Relative Residual mode 5","EMTF Relative Residual mode 5", 700, -2, 5);
    //TH1F *EMTF_Relative_Residual_mode_3 = new TH1F("EMTF Relative Residual mode 3","EMTF Relative Residual mode 3", 700, -2, 5);
    
    TH1F *CSCTF_Relative_Residual_all_modes = new TH1F("CSCTF Relative Residual all modes","CSCTF Relative Residual all modes", 700, -2, 5);
    //TH1F *CSCTF_Relative_Residual_mode_15 = new TH1F("CSCTF Relative Residual mode 15","CSCTF Relative Residual mode 15", 700, -2, 5);
    //TH1F *CSCTF_Relative_Residual_mode_14 = new TH1F("CSCTF Relative Residual mode 14","CSCTF Relative Residual mode 14", 700, -2, 5);
    //TH1F *CSCTF_Relative_Residual_mode_13 = new TH1F("CSCTF Relative Residual mode 13","CSCTF Relative Residual mode 13", 700, -2, 5);
    //TH1F *CSCTF_Relative_Residual_mode_12 = new TH1F("CSCTF Relative Residual mode 12","CSCTF Relative Residual mode 12", 700, -2, 5);
    //TH1F *CSCTF_Relative_Residual_mode_11 = new TH1F("CSCTF Relative Residual mode 11","CSCTF Relative Residual mode 11", 700, -2, 5);
    //TH1F *CSCTF_Relative_Residual_mode_10 = new TH1F("CSCTF Relative Residual mode 10","CSCTF Relative Residual mode 10", 700, -2, 5);
    //TH1F *CSCTF_Relative_Residual_mode_9 = new TH1F("CSCTF Relative Residual mode 9","CSCTF Relative Residual mode 9", 700, -2, 5);
    //TH1F *CSCTF_Relative_Residual_mode_7 = new TH1F("CSCTF Relative Residual mode 7","CSCTF Relative Residual mode 7", 700, -2, 5);
    //TH1F *CSCTF_Relative_Residual_mode_6 = new TH1F("CSCTF Relative Residual mode 6","CSCTF Relative Residual mode 6", 700, -2, 5);
    //TH1F *CSCTF_Relative_Residual_mode_5 = new TH1F("CSCTF Relative Residual mode 5","CSCTF Relative Residual mode 5", 700, -2, 5);
    //TH1F *CSCTF_Relative_Residual_mode_3 = new TH1F("CSCTF Relative Residual mode 3","CSCTF Relative Residual mode 3", 700, -2, 5);
    
    
    TH1F *EMTF_Relative_Residual_disagree_modes = new TH1F("EMTF Relative Residual disagree modes","EMTF Relative Residual disagree modes", 700, -2, 5);
    TH1F *EMTF_Relative_Residual_agree_modes = new TH1F("EMTF Relative Residual agree modes","EMTF Relative Residual agree modes", 700, -2, 5);
    
    TH1F *CSCTF_Relative_Residual_disagree_modes = new TH1F("CSCTF Relative Residual disagree modes","CSCTF Relative Residual disagree modes", 700, -2, 5);
    TH1F *CSCTF_Relative_Residual_agree_modes = new TH1F("CSCTF Relative Residual agree modes","CSCTF Relative Residual agree modes", 700, -2, 5);
    
    //-------------------------------------------------------
    //pT and eta distribution of all disagree and agree modes
    //-------------------------------------------------------
    TH1F *EMTF_pT_disagree_modes = new TH1F("EMTF pT disagree modes","EMTF pT disagree modes",300,0,300);
    TH1F *EMTF_pT_agree_modes = new TH1F("EMTF pT agree modes","EMTF pT agree modes",300,0,300);
    
    TH1F *CSCTF_pT_disagree_modes = new TH1F("CSCTF pT disagree modes","CSCTF pT disagree modes",150,0,150);
    TH1F *CSCTF_pT_agree_modes = new TH1F("CSCTF pT agree modes","CSCTF pT agree modes",150,0,150);
    //eta are same, can also choose to fill EMTF or CSCTF eta
    TH1F *RECO_eta_disagree_modes = new TH1F("RECO eta disagree modes","RECO eta disagree modes",24,0,2.4);
    TH1F *RECO_eta_agree_modes = new TH1F("RECO eta agree modes","RECO eta agree modes",24,0,2.4);
    
    //--------------------------------------------------------
    //pT and eta distribution of 3-station,4-station mode pair
    //--------------------------------------------------------
    //pms 1
    TH1F *EMTF_pT_EMTF_mode_7_CSCTF_mode_15 = new TH1F("EMTF pT when EMTF mode=7, CSCTF mode=15","EMTF pT when EMTF mode=7, CSCTF mode=15",300,0,300);
    TH1F *CSCTF_pT_EMTF_mode_7_CSCTF_mode_15 = new TH1F("CSCTF pT when EMTF mode=7, CSCTF mode=15","CSCTF pT when EMTF mode=7, CSCTF mode=15",150,0,150);
    TH1F *RECO_eta_EMTF_mode_7_CSCTF_mode_15 = new TH1F("RECO eta when EMTF mode=7, CSCTF mode=15","RECO eta when EMTF mode=7, CSCTF mode=15",24,0,2.4);
    
    TH1F *EMTF_pT_EMTF_mode_15_CSCTF_mode_7 = new TH1F("EMTF pT when EMTF mode=15, CSCTF mode=7","EMTF pT when EMTF mode=15, CSCTF mode=7",300,0,300);
    TH1F *CSCTF_pT_EMTF_mode_15_CSCTF_mode_7 = new TH1F("CSCTF pT when EMTF mode=15, CSCTF mode=7","CSCTF pT when EMTF mode=15, CSCTF mode=7",150,0,150);
    TH1F *RECO_eta_EMTF_mode_15_CSCTF_mode_7 = new TH1F("RECO eta when EMTF mode=15, CSCTF mode=7","RECO eta when EMTF mode=15, CSCTF mode=7",24,0,2.4);
    //pms 2
    TH1F *EMTF_pT_EMTF_mode_11_CSCTF_mode_15 = new TH1F("EMTF pT when EMTF mode=11, CSCTF mode=15","EMTF pT when EMTF mode=11, CSCTF mode=15",300,0,300);
    TH1F *CSCTF_pT_EMTF_mode_11_CSCTF_mode_15 = new TH1F("CSCTF pT when EMTF mode=11, CSCTF mode=15","CSCTF pT when EMTF mode=11, CSCTF mode=15",150,0,150);
    TH1F *RECO_eta_EMTF_mode_11_CSCTF_mode_15 = new TH1F("RECO eta when EMTF mode=11, CSCTF mode=15","RECO eta when EMTF mode=11, CSCTF mode=15",24,0,2.4);
    
    TH1F *EMTF_pT_EMTF_mode_15_CSCTF_mode_11 = new TH1F("EMTF pT when EMTF mode=15, CSCTF mode=11","EMTF pT when EMTF mode=15, CSCTF mode=11",300,0,300);
    TH1F *CSCTF_pT_EMTF_mode_15_CSCTF_mode_11 = new TH1F("CSCTF pT when EMTF mode=15, CSCTF mode=11","CSCTF pT when EMTF mode=15, CSCTF mode=11",150,0,150);
    TH1F *RECO_eta_EMTF_mode_15_CSCTF_mode_11 = new TH1F("RECO eta when EMTF mode=15, CSCTF mode=11","RECO eta when EMTF mode=15, CSCTF mode=11",24,0,2.4);
    //pms 3
    TH1F *EMTF_pT_EMTF_mode_13_CSCTF_mode_15 = new TH1F("EMTF pT when EMTF mode=13, CSCTF mode=15","EMTF pT when EMTF mode=13, CSCTF mode=15",300,0,300);
    TH1F *CSCTF_pT_EMTF_mode_13_CSCTF_mode_15 = new TH1F("CSCTF pT when EMTF mode=13, CSCTF mode=15","CSCTF pT when EMTF mode=13, CSCTF mode=15",150,0,150);
    TH1F *RECO_eta_EMTF_mode_13_CSCTF_mode_15 = new TH1F("RECO eta when EMTF mode=13, CSCTF mode=15","RECO eta when EMTF mode=13, CSCTF mode=15",24,0,2.4);
    
    TH1F *EMTF_pT_EMTF_mode_15_CSCTF_mode_13 = new TH1F("EMTF pT when EMTF mode=15, CSCTF mode=13","EMTF pT when EMTF mode=15, CSCTF mode=13",300,0,300);
    TH1F *CSCTF_pT_EMTF_mode_15_CSCTF_mode_13 = new TH1F("CSCTF pT when EMTF mode=15, CSCTF mode=13","CSCTF pT when EMTF mode=15, CSCTF mode=13",150,0,150);
    TH1F *RECO_eta_EMTF_mode_15_CSCTF_mode_13 = new TH1F("RECO eta when EMTF mode=15, CSCTF mode=13","RECO eta when EMTF mode=15, CSCTF mode=13",24,0,2.4);
    //pm 4
    TH1F *EMTF_pT_EMTF_mode_14_CSCTF_mode_15 = new TH1F("EMTF pT when EMTF mode=14, CSCTF mode=15","EMTF pT when EMTF mode=14, CSCTF mode=15",300,0,300);
    TH1F *CSCTF_pT_EMTF_mode_14_CSCTF_mode_15 = new TH1F("CSCTF pT when EMTF mode=14, CSCTF mode=15","CSCTF pT when EMTF mode=14, CSCTF mode=15",150,0,150);
    TH1F *RECO_eta_EMTF_mode_14_CSCTF_mode_15 = new TH1F("RECO eta when EMTF mode=14, CSCTF mode=15","RECO eta when EMTF mode=14, CSCTF mode=15",24,0,2.4);
    
    TH1F *EMTF_pT_EMTF_mode_15_CSCTF_mode_14 = new TH1F("EMTF pT when EMTF mode=15, CSCTF mode=14","EMTF pT when EMTF mode=15, CSCTF mode=14",300,0,300);
    TH1F *CSCTF_pT_EMTF_mode_15_CSCTF_mode_14 = new TH1F("CSCTF pT when EMTF mode=15, CSCTF mode=14","CSCTF pT when EMTF mode=15, CSCTF mode=14",150,0,150);
    TH1F *RECO_eta_EMTF_mode_15_CSCTF_mode_14 = new TH1F("RECO eta when EMTF mode=15, CSCTF mode=14","RECO eta when EMTF mode=15, CSCTF mode=14",24,0,2.4);
    
    //-----------------------------------------------------
    //pT and eta distribution of 2-station, 3-station mode
    //-----------------------------------------------------
    //pms 1
    TH1F *EMTF_pT_EMTF_mode_6_CSCTF_mode_14 = new TH1F("EMTF pT when EMTF mode=6, CSCTF mode=14","EMTF pT when EMTF mode=6, CSCTF mode=14",300,0,300);
    TH1F *CSCTF_pT_EMTF_mode_6_CSCTF_mode_14 = new TH1F("CSCTF pT when EMTF mode=6, CSCTF mode=14","CSCTF pT when EMTF mode=6, CSCTF mode=14",150,0,150);
    TH1F *RECO_eta_EMTF_mode_6_CSCTF_mode_14 = new TH1F("RECO eta when EMTF mode=6, CSCTF mode=14","CSCTF pT when EMTF mode=6, CSCTF mode=14",24,0,2.4);
    
    TH1F *EMTF_pT_EMTF_mode_14_CSCTF_mode_6 = new TH1F("EMTF pT when EMTF mode=14, CSCTF mode=6","EMTF pT when EMTF mode=14, CSCTF mode=6",300,0,300);
    TH1F *CSCTF_pT_EMTF_mode_14_CSCTF_mode_6 = new TH1F("CSCTF pT when EMTF mode=14, CSCTF mode=6","CSCTF pT when EMTF mode=14, CSCTF mode=6",150,0,150);
    TH1F *RECO_eta_EMTF_mode_14_CSCTF_mode_6 = new TH1F("RECO eta when EMTF mode=14, CSCTF mode=6","CSCTF pT when EMTF mode=14, CSCTF mode=6",24,0,2.4);
    //pms 1
    TH1F *EMTF_pT_EMTF_mode_5_CSCTF_mode_13 = new TH1F("EMTF pT when EMTF mode=5, CSCTF mode=13","EMTF pT when EMTF mode=5, CSCTF mode=13",300,0,300);
    TH1F *CSCTF_pT_EMTF_mode_5_CSCTF_mode_13 = new TH1F("CSCTF pT when EMTF mode=5, CSCTF mode=13","CSCTF pT when EMTF mode=5, CSCTF mode=13",150,0,150);
    TH1F *RECO_eta_EMTF_mode_5_CSCTF_mode_13 = new TH1F("RECO eta when EMTF mode=5, CSCTF mode=13","CSCTF pT when EMTF mode=5, CSCTF mode=13",24,0,2.4);
    
    TH1F *EMTF_pT_EMTF_mode_13_CSCTF_mode_5 = new TH1F("EMTF pT when EMTF mode=13, CSCTF mode=5","EMTF pT when EMTF mode=13, CSCTF mode=5",300,0,300);
    TH1F *CSCTF_pT_EMTF_mode_13_CSCTF_mode_5 = new TH1F("CSCTF pT when EMTF mode=13, CSCTF mode=5","CSCTF pT when EMTF mode=13, CSCTF mode=5",150,0,150);
    TH1F *RECO_eta_EMTF_mode_13_CSCTF_mode_5 = new TH1F("RECO eta when EMTF mode=13, CSCTF mode=5","CSCTF pT when EMTF mode=13, CSCTF mode=5",24,0,2.4);
    ...
    */
    ...
    for(Long64_t iEntry=0;iEntry<numEvents;iEntry++){
        ...
        if(TrkPt->size()!=0 && Leg_TrkPt->size()!=0 && RecoPt->size()!=0){
            ...
            while(matched_emtf_tracks!= TrkPt->size() && matched_emtf_tracks!=RecoPt->size()){
                ...
                //===========================
                //=add to v3 for LCT matching
                //===========================
                
                //further event selection
                if(min_dR_emtf_reco < 0.25 && trkBx[min_dR_itrk]==0 && fabs(recoEta[min_dR_jreco])>=1.2 && fabs(recoEta[min_dR_jreco])<=2.4){
            ...
            */
            ...
            while(matched_csctf_tracks!= Leg_TrkPt->size() && matched_csctf_tracks!=RecoPt->size()){
                ...
                //further event selection
                if(min_dR_csctf_reco < 0.25 && leg_trkBx[min_dR_kleg_trk]==0 && fabs(recoEta[min_dR_jreco])>=1.2 && fabs(recoEta[min_dR_jreco])<=2.4){
                    //more detailed mark
            ...
            for(int itrk=0;itrk<TrkPt->size();itrk++){
                ...
                for(int kleg_trk=0;kleg_trk<Leg_TrkPt->size();kleg_trk++){
                    ...
                    if(trk_flag[itrk]==leg_trk_flag[kleg_trk] && itrk<8 && kleg_trk<8){
                        ...
                        //=========================
                        //add @08162016 by Wei Shi
                        //=========================
                        //change from version3 for relative residual definition reco/emtf -1
                        double EMTF_Residual = -1/recoPt[selected_jreco]+1/trkPt[itrk];
                        double CSCTF_Residual = -1/recoPt[selected_jreco]+1/leg_trkPt[kleg_trk];
                        double EMTF_Relative_Residual = EMTF_Residual/(1/recoPt[selected_jreco]);
                        double CSCTF_Relative_Residual = CSCTF_Residual/(1/recoPt[selected_jreco]);
                        
                        //all modes
                        EMTF_Relative_Residual_all_modes->Fill(EMTF_Relative_Residual);
                        CSCTF_Relative_Residual_all_modes->Fill(CSCTF_Relative_Residual);
                        
                        //disagree/agree modes
                        if(trkMode[itrk] != leg_trkMode[kleg_trk]){
                            EMTF_Relative_Residual_disagree_modes->Fill(EMTF_Relative_Residual);
                            CSCTF_Relative_Residual_disagree_modes->Fill(CSCTF_Relative_Residual);
                            EMTF_pT_disagree_modes->Fill(trkPt[itrk]);
                            CSCTF_pT_disagree_modes->Fill(leg_trkPt[kleg_trk]);
                            RECO_eta_disagree_modes->Fill(fabs(recoEta[selected_jreco]));
                        }
                        else{
                            EMTF_Relative_Residual_agree_modes->Fill(EMTF_Relative_Residual);
                            CSCTF_Relative_Residual_agree_modes->Fill(CSCTF_Relative_Residual);
                            EMTF_pT_agree_modes->Fill(trkPt[itrk]);
                            CSCTF_pT_agree_modes->Fill(leg_trkPt[kleg_trk]);
                            RECO_eta_agree_modes->Fill(fabs(recoEta[selected_jreco]));
                        }
                        
                        //08152016@debug starts: print out LCTs eta and phi
                        //{{{
                        /*
                        for(int itrkLct=0; itrkLct<4; itrkLct++) {
                            
                            if (trkLct_station[itrk][itrkLct]>0 ) {
                                
                                //EMTF_stations_EMTF_mode_7_CSCTF_mode_15->Fill(trkLct_station[itrk][itrkLct]);
                                //itrkLct_flag[trkLct_station[itrk][itrkLct]-1]=1;
                                cout<<"trkLct #"<<itrkLct<<" in station: "<<trkLct_station[itrk][itrkLct]<<", eta= "<<trkLct_eta[itrk][itrkLct]<<", phi= "<<trkLct_globPhi[itrk][itrkLct]<<endl;
                        ...
                        for(int kleg_trkLct=0; kleg_trkLct<4; kleg_trkLct++) {
                            ...
                            if (leg_trkLct_station[kleg_trk][kleg_trkLct]>0 ) {
                                ...
                                cout<<"leg_trkLct #"<<kleg_trkLct<<" in station: "<<leg_trkLct_station[kleg_trk][kleg_trkLct]<<", eta= "<<leg_trkLct_eta[kleg_trk][kleg_trkLct]<<", phi= "<<leg_trkLct_globPhi[kleg_trk][kleg_trkLct]<<endl;
                        ...
                        for(int jrecoCscSeg=0; jrecoCscSeg<16;jrecoCscSeg++){
                            ...
                            if (recoCscSeg_station[selected_jreco][jrecoCscSeg]>0 ) {
                                ...
                                cout<<"recoCscSeg #"<<jrecoCscSeg<<" in station: "<<recoCscSeg_station[selected_jreco][jrecoCscSeg]<<", eta= "<<recoCscSeg_glob_eta[selected_jreco][jrecoCscSeg]<<", phi= "<<recoCscSeg_glob_phi[selected_jreco][jrecoCscSeg]<<endl;
                        ...
                        */
                        ...
                        if(trkMode[itrk]==7 && leg_trkMode[kleg_trk]==15){
                            ...
                            //add @08162016 by Wei Shi
                            //-------------------------
                            EMTF_pT_EMTF_mode_7_CSCTF_mode_15->Fill(trkPt[itrk]);
                            CSCTF_pT_EMTF_mode_7_CSCTF_mode_15->Fill(leg_trkPt[kleg_trk]);
                            RECO_eta_EMTF_mode_7_CSCTF_mode_15->Fill(fabs(recoEta[selected_jreco]));
                        ...
                        if(trkMode[itrk]==15 && leg_trkMode[kleg_trk]==7){
                            ...
                            //add @08162016 by Wei Shi
                            //-------------------------
                            EMTF_pT_EMTF_mode_15_CSCTF_mode_7->Fill(trkPt[itrk]);
                            CSCTF_pT_EMTF_mode_15_CSCTF_mode_7->Fill(leg_trkPt[kleg_trk]);
                            RECO_eta_EMTF_mode_15_CSCTF_mode_7->Fill(fabs(recoEta[selected_jreco]));
                        ...
                        if(trkMode[itrk]==11 && leg_trkMode[kleg_trk]==15){
                            ...
                            EMTF_pT_EMTF_mode_11_CSCTF_mode_15->Fill(trkPt[itrk]);
                            CSCTF_pT_EMTF_mode_11_CSCTF_mode_15->Fill(leg_trkPt[kleg_trk]);
                            RECO_eta_EMTF_mode_11_CSCTF_mode_15->Fill(fabs(recoEta[selected_jreco]));
                        ...
                        if(trkMode[itrk]==15 && leg_trkMode[kleg_trk]==11){
                            ...
                            EMTF_pT_EMTF_mode_15_CSCTF_mode_11->Fill(trkPt[itrk]);
                            CSCTF_pT_EMTF_mode_15_CSCTF_mode_11->Fill(leg_trkPt[kleg_trk]);
                            RECO_eta_EMTF_mode_15_CSCTF_mode_11->Fill(fabs(recoEta[selected_jreco]));
                        ...
                        if(trkMode[itrk]==13 && leg_trkMode[kleg_trk]==15){
                            ...
                            EMTF_pT_EMTF_mode_13_CSCTF_mode_15->Fill(trkPt[itrk]);
                            CSCTF_pT_EMTF_mode_13_CSCTF_mode_15->Fill(leg_trkPt[kleg_trk]);
                            RECO_eta_EMTF_mode_13_CSCTF_mode_15->Fill(fabs(recoEta[selected_jreco]));
                        ...
                        if(trkMode[itrk]==15 && leg_trkMode[kleg_trk]==13){
                            ...
                            EMTF_pT_EMTF_mode_15_CSCTF_mode_13->Fill(trkPt[itrk]);
                            CSCTF_pT_EMTF_mode_15_CSCTF_mode_13->Fill(leg_trkPt[kleg_trk]);
                            RECO_eta_EMTF_mode_15_CSCTF_mode_13->Fill(fabs(recoEta[selected_jreco]));
                        ...
                        if(trkMode[itrk]==14 && leg_trkMode[kleg_trk]==15){
                            ...
                            EMTF_pT_EMTF_mode_14_CSCTF_mode_15->Fill(trkPt[itrk]);
                            CSCTF_pT_EMTF_mode_14_CSCTF_mode_15->Fill(leg_trkPt[kleg_trk]);
                            RECO_eta_EMTF_mode_14_CSCTF_mode_15->Fill(fabs(recoEta[selected_jreco]));
                        ...
                        if(trkMode[itrk]==15 && leg_trkMode[kleg_trk]==14){
                            ...
                            EMTF_pT_EMTF_mode_15_CSCTF_mode_14->Fill(trkPt[itrk]);
                            CSCTF_pT_EMTF_mode_15_CSCTF_mode_14->Fill(leg_trkPt[kleg_trk]);
                            RECO_eta_EMTF_mode_15_CSCTF_mode_14->Fill(fabs(recoEta[selected_jreco]));
                        ...
                        if(trkMode[itrk]==5 && leg_trkMode[kleg_trk]==13){
                            ...
                            EMTF_pT_EMTF_mode_5_CSCTF_mode_13->Fill(trkPt[itrk]);
                            CSCTF_pT_EMTF_mode_5_CSCTF_mode_13->Fill(leg_trkPt[kleg_trk]);
                            RECO_eta_EMTF_mode_5_CSCTF_mode_13->Fill(fabs(recoEta[selected_jreco]));
                        ...
                        if(trkMode[itrk]==13 && leg_trkMode[kleg_trk]==5){
                            ...
                            EMTF_pT_EMTF_mode_13_CSCTF_mode_5->Fill(trkPt[itrk]);
                            CSCTF_pT_EMTF_mode_13_CSCTF_mode_5->Fill(leg_trkPt[kleg_trk]);
                            RECO_eta_EMTF_mode_13_CSCTF_mode_5->Fill(fabs(recoEta[selected_jreco]));
                        ...
                        if(trkMode[itrk]==6 && leg_trkMode[kleg_trk]==14){
                            ...
                            EMTF_pT_EMTF_mode_6_CSCTF_mode_14->Fill(trkPt[itrk]);
                            CSCTF_pT_EMTF_mode_6_CSCTF_mode_14->Fill(leg_trkPt[kleg_trk]);
                            RECO_eta_EMTF_mode_6_CSCTF_mode_14->Fill(fabs(recoEta[selected_jreco]));
                        ...
                        if(trkMode[itrk]==14 && leg_trkMode[kleg_trk]==6){
                            ...
                            EMTF_pT_EMTF_mode_14_CSCTF_mode_6->Fill(trkPt[itrk]);
                            CSCTF_pT_EMTF_mode_14_CSCTF_mode_6->Fill(leg_trkPt[kleg_trk]);
                            RECO_eta_EMTF_mode_14_CSCTF_mode_6->Fill(fabs(recoEta[selected_jreco]));
    ...
    TFile myPlot("/afs/cern.ch/work/w/wshi/public/EMTFpTResolution/TrackBuild_v4_274440_443_lxplus.root","RECREATE");
    //TFile myPlot("~/Documents/EMTFpTResolution/TrackBuild_v4_274440_443_local.root","RECREATE");
    ...
    //add @08162016 by Wei Shi
    EMTF_Relative_Residual_all_modes->Write();
    CSCTF_Relative_Residual_all_modes->Write();
    
    EMTF_Relative_Residual_disagree_modes->Write();
    EMTF_Relative_Residual_agree_modes->Write();
    
    CSCTF_Relative_Residual_disagree_modes->Write();
    CSCTF_Relative_Residual_agree_modes->Write();
    
    //-------------------------------------------------------
    //pT and eta distribution of all disagree and agree modes
    //-------------------------------------------------------
    EMTF_pT_disagree_modes->Write();
    EMTF_pT_agree_modes->Write();
    
    CSCTF_pT_disagree_modes->Write();
    CSCTF_pT_agree_modes->Write();
    //eta are same, can also choose to fill EMTF or CSCTF eta
    RECO_eta_disagree_modes->Write();
    RECO_eta_agree_modes->Write();
    
    //--------------------------------------------------------
    //pT and eta distribution of 3-station,4-station mode pair
    //--------------------------------------------------------
    //pms 1
    EMTF_pT_EMTF_mode_7_CSCTF_mode_15->Write();
    CSCTF_pT_EMTF_mode_7_CSCTF_mode_15->Write();
    RECO_eta_EMTF_mode_7_CSCTF_mode_15->Write();
    
    EMTF_pT_EMTF_mode_15_CSCTF_mode_7->Write();
    CSCTF_pT_EMTF_mode_15_CSCTF_mode_7->Write();
    RECO_eta_EMTF_mode_15_CSCTF_mode_7->Write();
    //pms 2
    EMTF_pT_EMTF_mode_11_CSCTF_mode_15->Write();
    CSCTF_pT_EMTF_mode_11_CSCTF_mode_15->Write();
    RECO_eta_EMTF_mode_11_CSCTF_mode_15->Write();
    
    EMTF_pT_EMTF_mode_15_CSCTF_mode_11->Write();
    CSCTF_pT_EMTF_mode_15_CSCTF_mode_11->Write();
    RECO_eta_EMTF_mode_15_CSCTF_mode_11->Write();
    //pms 3
    EMTF_pT_EMTF_mode_13_CSCTF_mode_15->Write();
    CSCTF_pT_EMTF_mode_13_CSCTF_mode_15->Write();
    RECO_eta_EMTF_mode_13_CSCTF_mode_15->Write();
    
    EMTF_pT_EMTF_mode_15_CSCTF_mode_13->Write();
    CSCTF_pT_EMTF_mode_15_CSCTF_mode_13->Write();
    RECO_eta_EMTF_mode_15_CSCTF_mode_13->Write();
    //pm 4
    EMTF_pT_EMTF_mode_14_CSCTF_mode_15->Write();
    CSCTF_pT_EMTF_mode_14_CSCTF_mode_15->Write();
    RECO_eta_EMTF_mode_14_CSCTF_mode_15->Write();
    
    EMTF_pT_EMTF_mode_15_CSCTF_mode_14->Write();
    CSCTF_pT_EMTF_mode_15_CSCTF_mode_14->Write();
    RECO_eta_EMTF_mode_15_CSCTF_mode_14->Write();
    
    //-----------------------------------------------------
    //pT and eta distribution of 2-station, 3-station mode
    //-----------------------------------------------------
    //pms 1
    EMTF_pT_EMTF_mode_6_CSCTF_mode_14->Write();
    CSCTF_pT_EMTF_mode_6_CSCTF_mode_14->Write();
    RECO_eta_EMTF_mode_6_CSCTF_mode_14->Write();
    
    EMTF_pT_EMTF_mode_14_CSCTF_mode_6->Write();
    CSCTF_pT_EMTF_mode_14_CSCTF_mode_6->Write();
    RECO_eta_EMTF_mode_14_CSCTF_mode_6->Write();
    //pms 1
    EMTF_pT_EMTF_mode_5_CSCTF_mode_13->Write();
    CSCTF_pT_EMTF_mode_5_CSCTF_mode_13->Write();
    RECO_eta_EMTF_mode_5_CSCTF_mode_13->Write();
    
    EMTF_pT_EMTF_mode_13_CSCTF_mode_5->Write();
    CSCTF_pT_EMTF_mode_13_CSCTF_mode_5->Write();
    RECO_eta_EMTF_mode_13_CSCTF_mode_5->Write();
     ...
     */
    //if(run_lxplus==1){
        cout<<"Save path:/afs/cern.ch/work/w/wshi/public/EMTFpTResolution/TrackBuild_v4_274440_443_lxplus.root"<<endl;
    //}
    //else if(run_lxplus==0){
        cout<<"Save path:~/Documents/EMTFpTResolution/TrackBuild_v4_274440_443_local.root"<<endl;
    //}
    //else{
      //  cout<<"Error: Fail to locate running machine!"<<endl;
    //}
