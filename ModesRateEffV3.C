...
//= Part I: I/O
//==============
//const double scale = 28500000./1566129;//ZB rate scale, unit: Hz
const double scale = 1.0;
TString ZeroBias_files[1] = { 
	  		    "NTuple_ZeroBias_FlatNtuple_Run_2018D_v2_2018_09_20_ZeroBias_2018_emul_ph_lut_v2.root"
                            //"NTuple_ZeroBias_FlatNtuple_Run_2018D_v2_2018_09_20_ZeroBias_2018_emul_ph_lut_v3.root"
...
TString SingleMu_files[1] = {
                           //"NTuple_SingleMuon_FlatNtuple_Run_2018D_v1_2018_09_19_SingleMuon_2018_emul_102X_ReReco_v1_321988_coord.root"//2018 default coordinate LUT  
	  		    //"NTuple_SingleMuon_FlatNtuple_Run_2018D_v1_2018_09_19_SingleMuon_2018_emul_102X_ReReco_v1_321988_bugFix.root"//Sep 2018 Fix LUT
...
TString scenario = "lutv2"; //Options: User choose to distinguish output file
//================================
//= Part II: Kinematics
//================================
//2017 modes
...
const int PT_UP = 50;//Reco pT range
const int PT_LOW = 0;
const int Log2_PT_UP = 9;//upper limit in single mu files
const int Log2_PT_LOW = 0;
const int PT_UP_TRIG = 50;//Trigger pT range for rate plots
const int PT_LOW_TRIG = 0;
const float ETA_UP = 2.40;//Reco eta @station 1
...
void ModesRateEffV3() {
  ...
  SMRecoPtTitle = SMRecoPtTitle + "RECO pT [" + Form("%d", PT_LOW)+", "+ Form("%d", PT_UP) + "]GeV, looseID, ReachStationOne, " + "abs(eta) [" + Form("%.2f", ETA_LOW)+", "+ Form("%.2f", ETA_UP) + "]";
  TString SMLogRecoPtTitle="";
  SMLogRecoPtTitle = SMLogRecoPtTitle + "Log2(RECO pT) [" + Form("%d", Log2_PT_LOW)+", "+ Form("%d", Log2_PT_UP) + "], looseID, ReachStationOne, " + "abs(eta) [" + Form("%.2f", ETA_LOW)+", "+ Form("%.2f", ETA_UP) + "]";
  ...
  double TrigPT[PT_UP_TRIG-PT_LOW_TRIG]={0};
  for(int i=PT_LOW_TRIG;i<PT_UP_TRIG;i++){
	  TrigPT[i]=i*1.0;
  }
  
  double Count[PT_UP_TRIG-PT_LOW_TRIG]={0};
  double CountSingleMu[PT_UP_TRIG-PT_LOW_TRIG]={0};
  double CountDoubleMuInc[PT_UP_TRIG-PT_LOW_TRIG]={0};
  double CountMuOpenInc[PT_UP_TRIG-PT_LOW_TRIG]={0};
	      ...
	      ( I("nRecoMuonsTrig")==1 && I("nRecoMuonsTrigCen")==1 && I("reco_trig_ID", ireco) < 0 ) 
		    ...
		    if( F("reco_pt", ireco) > 0 && 
			    ...
			    //***********************
		            //* IsRECOMatch && BX=0 *
		            //***********************
			    if( I("reco_dR_match_emu_nTrk", ireco) > 0 &&
			        I("trk_BX", I("reco_dR_match_emu_iTrk", ireco) ) == 0 
			      ){
				    
				    //SingleMu
				    for(int i=0;i<nSingleMu;i++){
					    if( I("trk_mode", I("reco_dR_match_emu_iTrk", ireco) ) == SingleMu[i]){
						    if(F("reco_pt", ireco) >= PT_LOW && F("reco_pt", ireco) <= PT_UP){
						    ...
						    //Plateau
						    //if( F("trk_pt", I("reco_dR_match_emu_iTrk", ireco)) > F("reco_pt", ireco) * 7.0/8.0 ||
						    if( F("trk_pt", I("reco_dR_match_emu_iTrk", ireco)) > 30 ){//SingleMu threshold
							    if(F("reco_pt", ireco) >= PT_LOW && F("reco_pt", ireco) <= PT_UP){
						    ...
						    //Unique match
						    if( I("reco_dR_match_emu_unique", ireco) == 1 ){
							    if(F("reco_pt", ireco) >= PT_LOW && F("reco_pt", ireco) <= PT_UP){
				    ...
				    for(int j=0;j<nDoubleMuInc;j++){
					    if( I("trk_mode", I("reco_dR_match_emu_iTrk", ireco) ) == DoubleMuInc[j]){
						    if(F("reco_pt", ireco) >= PT_LOW && F("reco_pt", ireco) <= PT_UP){
						    ...
						    //Plateau
						    //if( F("trk_pt", I("reco_dR_match_emu_iTrk", ireco)) > F("reco_pt", ireco) * 7.0/8.0 ||
						    if( F("trk_pt", I("reco_dR_match_emu_iTrk", ireco)) > 30 ){
							    if(F("reco_pt", ireco) >= PT_LOW && F("reco_pt", ireco) <= PT_UP){
				    ...
				    for(int k=0;k<nMuOpenInc;k++){
					    if( I("trk_mode", I("reco_dR_match_emu_iTrk", ireco) ) == MuOpenInc[k]){
						    if(F("reco_pt", ireco) >= PT_LOW && F("reco_pt", ireco) <= PT_UP){
  ...
  outFile = outFile + "_" + scenario + "_eta_" + Form("%.2f", ETA_LOW) + "_" + Form("%.2f", ETA_UP) + ".root";
  ...
  double Rate[PT_UP_TRIG-PT_LOW_TRIG]={0};//A.U.
  double RateSingleMu[PT_UP_TRIG-PT_LOW_TRIG]={0};
  double RateDoubleMuInc[PT_UP_TRIG-PT_LOW_TRIG]={0};
  double RateMuOpenInc[PT_UP_TRIG-PT_LOW_TRIG]={0};
  ...
  //Rate plots
  TH1F *HRate = new TH1F("HRate", "Rate", PT_UP_TRIG-PT_LOW_TRIG, PT_LOW_TRIG, PT_UP_TRIG); HRate->GetXaxis()->SetTitle("Trig pT[GeV]"); HRate->GetYaxis()->SetTitle("Rate[A.U.]");
  TH1F *HRateSingleMu = new TH1F("HRateSingleMu", "SingleMu", PT_UP_TRIG-PT_LOW_TRIG, PT_LOW_TRIG, PT_UP_TRIG); HRateSingleMu->GetXaxis()->SetTitle("Trig pT[GeV]"); HRateSingleMu->GetYaxis()->SetTitle("Rate[A.U.]");//SingleMu only
  TH1F *HRateDoubleMuInc = new TH1F("HRateDoubleMuInc", "DoubleMu Inclusive", PT_UP_TRIG-PT_LOW_TRIG, PT_LOW_TRIG, PT_UP_TRIG); HRateDoubleMuInc->GetXaxis()->SetTitle("Trig pT[GeV]"); HRateDoubleMuInc->GetYaxis()->SetTitle("Rate[A.U.]");//DoubleMu Inc
  TH1F *HRateMuOpenInc = new TH1F("HRateMuOpenInc", "MuOpen Inclusive", PT_UP_TRIG-PT_LOW_TRIG, PT_LOW_TRIG, PT_UP_TRIG); HRateMuOpenInc->GetXaxis()->SetTitle("Trig pT[GeV]"); HRateMuOpenInc->GetYaxis()->SetTitle("Rate[A.U.]");//MuOpen Inc
  //Rate Normalized w.r.t. first bin
  TH1F *HRateNorm = new TH1F("HRateNorm", "Rate", PT_UP_TRIG-PT_LOW_TRIG, PT_LOW_TRIG, PT_UP_TRIG); HRateNorm->GetXaxis()->SetTitle("Trig pT[GeV]"); HRateNorm->GetYaxis()->SetTitle("Rate[A.U.]");
  TH1F *HRateSingleMuNorm = new TH1F("HRateSingleMuNorm", "SingleMu", PT_UP_TRIG-PT_LOW_TRIG, PT_LOW_TRIG, PT_UP_TRIG); HRateSingleMuNorm->GetXaxis()->SetTitle("Trig pT[GeV]"); HRateSingleMuNorm->GetYaxis()->SetTitle("Rate[A.U.]");//SingleMu only
  TH1F *HRateDoubleMuIncNorm = new TH1F("HRateDoubleMuIncNorm", "DoubleMu Inclusive", PT_UP_TRIG-PT_LOW_TRIG, PT_LOW_TRIG, PT_UP_TRIG); HRateDoubleMuIncNorm->GetXaxis()->SetTitle("Trig pT[GeV]"); HRateDoubleMuIncNorm->GetYaxis()->SetTitle("Rate[A.U.]");//DoubleMu Inc
  TH1F *HRateMuOpenIncNorm = new TH1F("HRateMuOpenIncNorm", "MuOpen Inclusive", PT_UP_TRIG-PT_LOW_TRIG, PT_LOW_TRIG, PT_UP_TRIG); HRateMuOpenIncNorm->GetXaxis()->SetTitle("Trig pT[GeV]"); HRateMuOpenIncNorm->GetYaxis()->SetTitle("Rate[A.U.]");//MuOpen Inc
  ...
  for(int i=PT_LOW_TRIG;i<PT_UP_TRIG;i++){
	  ...
	  HRateMuOpenInc->Fill(TrigPT[i],RateMuOpenInc[i]);
          ...
          HRateNorm->Fill(TrigPT[i],Rate[i]*1.0/Rate[PT_LOW_TRIG]);
          HRateSingleMuNorm->Fill(TrigPT[i],RateSingleMu[i]*1.0/RateSingleMu[PT_LOW_TRIG]);
          HRateDoubleMuIncNorm->Fill(TrigPT[i],RateDoubleMuInc[i]*1.0/RateDoubleMuInc[PT_LOW_TRIG]);
          HRateMuOpenIncNorm->Fill(TrigPT[i],RateMuOpenInc[i]*1.0/RateMuOpenInc[PT_LOW_TRIG]);
  }
  ...
  HRateNorm->Write();
  HRateSingleMuNorm->Write();
  HRateDoubleMuIncNorm->Write();
  HRateMuOpenIncNorm->Write();	 
